use crate::{context::WrappedContext, metrics, utils::{response_error}};
use log::{info, error};
use serde::Serialize;
...
fn filter_and_sort_stakes(records: &mut Vec<StakingChange>) {
    records.retain(|stake| (stake.next_stake_sol * LAMPORTS_PER_SOL) as f64 - stake.current_stake as f64 != 0.0);
...
async fn get_planned_stakes(
    ...
    for validator_score in scores.values().into_iter() {
        let should_have = validator_score.target_stake_algo + validator_score.target_stake_mnde + validator_score.target_stake_msol;
        ...
        match stake_info {
            ...
            None => {
                error!("Couldn't find current stake info for {}", validator_score.vote_account);
                continue;
            }
        };
    }
