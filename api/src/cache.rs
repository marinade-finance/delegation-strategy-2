...
use store::dto::{
    ClusterStats, CommissionRecord, ScoringRunRecord, UptimeRecord, ValidatorRecord, ValidatorScoreRecord, ValidatorsAggregated, VersionRecord
};
...
pub fn spawn_cache_warmer(
    ...
    tokio::spawn(async move {
        ...
        loop {
            let redis_check = redis_cache::check_redis_timestamp(
                &redis_client,
                &last_timestamp,
                redis_tag.clone(),
            )
            ...
            if !redis_check.ok().unwrap() {
                ...
                if let Ok(timestamp) =
                    redis_cache::get_redis_timestamp(&redis_client, redis_tag.clone()).await
                {
