...
validators$eligible_stake_vemnde[validators$max_commission > ELIGIBILITY_MNDE_STAKE_MAX_COMMISSION] <- 0
validators$eligible_stake_vemnde[validators$minimum_stake < ELIGIBILITY_MNDE_STAKE_MIN_STAKE] <- 0
validators$eligible_stake_vemnde[validators$score < min_score_in_algo_set * ELIGIBILITY_MNDE_SCORE_THRESHOLD_MULTIPLIER] <- 0
...
STAKE_CONTROL_MNDE_SOL <- TOTAL_STAKE * STAKE_CONTROL_MNDE * (1 - vemnde_overflow_power)
STAKE_CONTROL_MNDE_OVERFLOW_SOL <- vemnde_overflow_power * TOTAL_STAKE * STAKE_CONTROL_MNDE
...
validators$target_stake_vemnde <- round(validators$vemnde_power * STAKE_CONTROL_MNDE_SOL)
