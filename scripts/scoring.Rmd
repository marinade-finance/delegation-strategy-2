...
author: Marinade Finance - Delegation Strategy
output:
  html_document:
    toc: true
    toc_float: true
...
plot1 <- ggplot(df, aes(x=df$x, y=df$score)) +
...
plot2 <- ggplot(df, aes(x=df$x, y=df$score)) +
  geom_bar(stat = "identity", width = 1, colour = "transparent", fill = df$fill_mnde) +
  ylim(0, 1) +
  labs(title="MNDE voted validators", x = "Validators", y = "Score") +
  scale_x_discrete(labels = NULL) +
  theme_minimal()
