...
pub struct StakersParams {
    ...
    #[structopt(
        long = "from-epoch",
        help = "Backfill: query all epochs >= this value in a single run. Overrides --epoch."
    )]
    ...
    #[structopt(
        ...
        help = "Fail instead of skipping when stakers data is still missing this many hours after the target epoch ended (approximate; assumes ~400ms slots). 0 disables the check. Ignored when --epoch or --from-epoch is set (backfill).",
        default_value = "12"
    )]
...
async fn query_validator_stakers(
    ...
    let query = format!(
        "SELECT epoch, vote_account, \
                COUNT(DISTINCT withdraw_authority) AS unique_stakers, \
                CAST(SUM(active) AS INT64) AS active_stake \
         FROM `{project_table_name}` \
         WHERE active > 0 AND vote_account IS NOT NULL AND {epoch_where} \
         GROUP BY epoch, vote_account"
    );
    ...
    info!("Retrieved {row_count} rows from BigQuery");
...
pub fn collect_validator_stakers_info(
    ...
    // Backfill mode: pull every epoch >= from_epoch in one run and always emit.
    if let Some(from_epoch) = stakers_params.from_epoch {
        info!("Backfilling stakers from epoch {from_epoch}");
        let stakers = runtime.block_on(async {
            let bq_client = create_bigquery_client().await?;
            query_validator_stakers(
                &bq_client,
                &stakers_params.gcp_table_name,
