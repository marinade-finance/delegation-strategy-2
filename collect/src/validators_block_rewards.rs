...
pub fn collect_validator_block_rewards_info(
    ...
    if block_rewards.len() <= rewards_params.loading_limit as usize {
        ...
        if rewards_params.epoch.is_none()
            && rewards_params.max_data_delay_hours > 0
        ...
        warn!("No data found for epoch {looking_at_epoch} (~{hours_since_epoch_end}h after epoch end). May not be available yet; will retry next run. BigQuery returned {} rows.", block_rewards.len());
    } else {
        info!(
